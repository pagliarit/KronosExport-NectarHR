...
--last edit 6/6/2022 -- added 'LastHireDate' and filtered manager by primary email
...
--             where PersonToEffectDate <= @From_Timeframe
 ...
 --              where ToEffectiveDate <= @From_Timeframe 
    ...
    left outer join (select esi2.empno supnumber, vem2.EMailAddress supemail 
    ...
    left join vEMAIL vem2 on vp2.PersonIDNo = vem2.PersonIdNo and vem2.EmailPrimaryInd = 1  ) ManEmail on SUPERVISORNUM = ManEmail.supnumber --left join (SELECT rtrim(esi.EmpNo) as xempid, SUPERVISORFULLNAME supname ,SUPERVISORNUM  supid) managers on managers.xempid = manager_ID
